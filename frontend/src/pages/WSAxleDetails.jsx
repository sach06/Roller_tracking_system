...
const WSAxleDetails = () => {
    ...
    const fetchLifecycleData = async () => {
        try {
            const response = await axios.get(`/api/processing/lifecycle/sleeve-by-id/${lifecycleId}`);
            // Note: need to ensure backend has a way to fetch by lifecycle only or handle the dummy ID
            // For now, I'll use the existing /api/processing/lifecycle/:id/:lifecycleId but with a dummy id if needed
            // Actually, I'll add a specific endpoint /api/ws/lifecycle/:lifecycleId for simpler WS access
            const resp = await axios.get(`/api/processing/lifecycle/dummy/${lifecycleId}`);
            if (resp.data.success) {
                setLifecycleData(resp.data.lifecycle);
            }
            setLoading(false);
        } catch (err) {
